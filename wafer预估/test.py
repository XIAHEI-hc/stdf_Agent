...
def evaluate_grid_points(points, step_x, step_y):
    """评估每个网点的覆盖区域状态"""
    grid_info = {}
    valid_count = 0  # 有效矩形计数器
    validity_list = []  # 有效性列表（1=有效，0=无效）
    ...
    for idx, (x, y) in enumerate(points):
        ...
        if count_effective == 4:
            grid_info[idx] = {'status': '内部', 'fill': 'black', 'valid': 1}
            valid_count += 1
            validity_list.append(1)
        elif count_effective == 0:
            grid_info[idx] = {'status': '外部', 'fill': None, 'valid': 0}
            validity_list.append(0)
        else:
            grid_info[idx] = {'status': '部分', 'fill': 'pink', 'valid': 1}
            valid_count += 1
            validity_list.append(1)
...
def save_to_excel(points, validity_list, filename="wafer_validity.xlsx"):
    """将网点坐标和有效性保存到Excel文件"""
    data = {
        "Point Index": range(len(points)),
        "X Coordinate": [p[0] for p in points],
        "Y Coordinate": [p[1] for p in points],
        "Validity (1=Valid)": validity_list
    }
...
if __name__ == "__main__":
    ...
    # 评估网点状态
    grid_status, valid_count, validity_list = evaluate_grid_points(spiral_points, step_size_x, step_size_y)

    # 输出结果
    print(f"生成的网点总数: {len(spiral_points)}")
    print(f"有效矩形数量: {valid_count}")
    print("\n前5个网点坐标和状态:")
    for i in range(5):
        print(
            f"网点{i}: 坐标({spiral_points[i][0]:.2f}, {spiral_points[i][1]:.2f}) - {grid_status[i]['status']} - 有效性: {grid_status[i]['valid']}")

    # 保存到Excel
    save_to_excel(spiral_points, validity_list)
